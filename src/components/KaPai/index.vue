<template>
  <div class="kapai">
    ...
    <div class="kapai_bottom">
      <p>{{ props.title }}</p>
      <div class="kia_bort_bot">
        <div class="D_bi_bott">
          <div class="left_bott">
            <div class="D_bi">
              <sectionVV size="18px" icon-class="DBi" />
            </div>
            <div class="jin">
        ...
        <div v-show="props.percentage !== ''" class="jian">{{ props.percentage }}</div>
      </div>
    </div>
  </div>
</template>
<script setup>
import sectionVV from '@/components/SvgIcon/index.vue'
...
.kapai {
  ...
  border-image: linear-gradient(#8d5cc7, #6496e7) 2 2 2 2;
  ...
  &:hover {
    transition: all 1s;
    border-image: linear-gradient(#3543fc, #c12fc7) 2 2 2 2;
    background: linear-gradient(0deg, #050133 0%, #540078 96.64%);
    .xiaoxingxing {
      z-index: 0;
